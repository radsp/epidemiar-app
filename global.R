...
report_woredas <- read_csv("data/amhara_woredas.csv") %>%
  filter(report == 1) %>% filter(woreda_name %in% wlist)
...
s2_amh <- subset(s2, (country %in% "Ethiopia") & (admin1 %in% "Amhara")) %>%
  subset(admin2 %in% c("Mekete", "Bugina", "Gidan"))
  

#read woreda information (metadata) file
woredas <- read_csv("data/amhara_woredas.csv")  %>% filter(woreda_name %in% wlist)

# Add woreda name
s2_amh$woreda <- NA
s2_amh$woreda<- woredas$woreda_name
s2_amh$woreda<- paste("woreda ", row.names(s2_amh))

# Add zone name
s2_amh$zone <- NA
s2_amh$zone<- woredas$zone
s2_amh$zone<- paste("zone ", row.names(s2_amh))

# WID
s2_amh$WID <- NA
s2_amh$WID<- woredas$WID
# s2_amh$WID<- 10000 + as.numeric(row.names(s2_amh))
...
overview_colors <- c("#d7301f", "#fc8d59", "#3182bd", "gray98")

pal <- colorFactor(overview_colors, 
                   domain = factor(c('High: Alerts for 2 or more weeks\n  in forecast period',
                                     'Moderate: Alert for any 1 week\n  in forecast period',
                                     'Low: No alerts', 'None'),
                                   levels = c('High: Alerts for 2 or more weeks\n  in forecast period',
                                              'Moderate: Alert for any 1 week\n  in forecast period',
                                              'Low: No alerts', 'None')))
